...
async def test_upload_profile_picture(client: AsyncClient, mocked_aws):
    ...
    response = await client.patch(
        ...
        files={"file": ("profile.jpg", BytesIO(image_bytes), "image/jpeg")},  # it actually tell https to transfer data in multipartite part
...
async def test_forgot_password_sends_email(client: AsyncClient):
    ...
    with patch(     #using this patch as a context manager 
        "routers.users.send_password_reset_email",  # Patch where it is used, not where it is defined. thats why i point it via users not directly emails_util.py
